...
export default function Layout({ children, currentPage = 'home', showLogin = false, isAdminMode = false }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Navigation - Only show if NOT in admin mode */}
      {!isAdminMode && (
                  ...
                  {navigation.map((item) => (
                    <div key={item.name}>
                      {item.dropdown ? (
                        ...
                        <Link
                          href={item.href}
                          className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-200"
                          onClick={() => setMobileMenuOpen(false)}
                        >
                          <item.icon className="h-4 w-4" />
                          <span>{item.name}</span>
      ...
      {/* Footer - Only show if NOT in admin mode */}
      {!isAdminMode && (
